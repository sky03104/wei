...
1. **帳號密碼**：✅ 已補上——首頁右上角「🔑 改密碼」按鈕（只在
   `BACKEND==='supabase'` 顯示，直接呼叫 Supabase Auth 的
   `updateUser()`，不用經過管理員）。現在 Supabase 那邊每個帳號都是
...
2. **新增帳號的 Edge Function**：✅ 已經寫好（`supabase/functions/admin-users/index.ts`）、
   部署過、實測過「新增帳號」跟「重設密碼」都成功。系統管理頁的
   按鈕完全沒改，`supabaseApi()` 依情況分流：改已存在帳號的角色/
   狀態/顯示名稱（有 `userId`）走一般的 `admin_update_user()` RPC，
   不需要 service role；建立全新帳號、重設密碼這兩件事才會呼叫這支
   Edge Function。

   部署過程中抓到一個真的 bug：Edge Function 第一版沒有加 CORS
   標頭，導致「重設密碼」實際上後端已經執行成功（密碼真的改了），
   但瀏覽器因為讀不到回應（沒有 `Access-Control-Allow-Origin`）誤判
   成失敗，畫面顯示失敗但實際成功了——這種「後端做完了、前端以為
   沒做」的狀況特別容易誤導人，已經補上 `corsHeaders` 跟明確處理
   `OPTIONS` 預檢請求修好，重新部署後兩個功能都正常。
3. **匯出 Excel**：✅ 已補上——`docs/index.html` 用 CDN 載入 ExcelJS
   （只有 `BACKEND==='supabase'` 才會用到），瀏覽器端現場組出 .xlsx
   檔案，樣式（粗體表頭、凍結首列、分隔黑列、小計粉紅底、+/- 紅字、
   分頁命名規則）都對照 `_writeLedgerSheet()` 做，回傳的形狀
   跟 GAS 版本一致，`downloadLedgerXlsx()` 沒有改動。用真的 `exceljs`
   套件（不是假的）在本機把產生的檔案讀回來驗證過樣式都寫對了。
   **這次是使用者要求先做起來看排版對不對，還沒決定要不要合併回
   `main`**——目前只留在這個分支，`BACKEND` 也還是只在這個分支的
   `config.js` 才是 `'supabase'`，`main` 完全沒被動到。
4. **孤兒資料**：✅ 已確認——使用者查證過，這是機台 18~20 留下的
   舊資料（曾經新增過這三台，後來刪掉了），對應的紀錄跟授權自然
   對不到現有機台，不是遺漏重要資料，維持現有「遷移時自動跳過」
   的處理方式即可，不需要手動補進 Supabase。
