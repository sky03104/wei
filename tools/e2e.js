...
async function main() {
  ...
  await check('首頁分頁籤（骰台／電子／加總）＋新增電子機台並記錄開分/洗分', async () => {
    ...
    // 盈虧金額要用開分－洗分算（不是骰台那套 in/out/prize，那套對電子機台永遠是 0），
    // 標籤也要跟詳細頁一致叫「盈虧金額」而不是「淨收益」。
    await page.click('button:has-text("📊 查詢報表")');
    await page.waitForSelector('.report-stats');
    const reportLabels = await page.locator('.report-stats .stat-label').allTextContents();
    assert(reportLabels.includes('開分') && reportLabels.includes('洗分') && reportLabels.includes('盈虧金額'),
      '電子機台報表頁應該顯示開分/洗分/盈虧金額，實際 ' + JSON.stringify(reportLabels));
    assert(!reportLabels.includes('入幣') && !reportLabels.includes('出幣') && !reportLabels.includes('活動成本') && !reportLabels.includes('淨收益'),
      '電子機台報表頁不該出現入幣/出幣/活動成本/淨收益，實際 ' + JSON.stringify(reportLabels));
    const reportStats = await page.locator('.report-stats .stat').allTextContents();
    const reportNet = reportStats.find((t) => t.indexOf('盈虧金額') >= 0);
    assert(reportNet && num(reportNet) === 300, '電子機台報表頁的盈虧金額應該是開分500－洗分200＝300，實際「' + reportNet + '」');
    await page.click('button:has-text("← 返回")');
    await page.waitForSelector('.detail-hero');
